...
func (oas TykOAS) Update(ctx context.Context, id, data string) error {
	...
	// Need to set API ID in the body too for update to work
	result, err := jsonparser.Set([]byte(data), []byte(quotedID), "x-tyk-api-gateway", "info", "id")
	if err != nil {
		return err
	}

	resp, err := client.Put(ctx, client.Join(endpointOAS, id), bytes.NewBuffer(result))
	if err != nil {
		return err
	}
